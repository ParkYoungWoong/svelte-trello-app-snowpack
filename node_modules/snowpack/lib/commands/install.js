...
const resolve_remote_js_1 = require("../resolve-remote.js");
...
async function getInstallTargets(config, scannedFiles) {
    ...
    if (webDependencies) {
        installTargets.push(...scan_imports_js_1.scanDepList(Object.keys(webDependencies), cwd));
    }
...
async function run({ config, lockfile, installTargets, shouldWriteLockfile, shouldPrintStats, }) {
    ...
    if (webDependencies && Object.keys(webDependencies).length > 0) {
        newLockfile = await resolve_remote_js_1.resolveTargetsFromRemoteCDN(lockfile, config).catch((err) => {
